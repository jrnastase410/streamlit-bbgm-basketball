...
df_team1 = df_import[df_import['team'] == team1][['player', 'pid', 'ovr', 'value']]
df_team2 = df_import[df_import['team'] == team2][['player', 'pid', 'ovr', 'value']]
...
with col1:
    edited_df_team1 = st.data_editor(df_team1.round({'value': 1}), hide_index=True, key='df_team1')
with col2:
    edited_df_team2 = st.data_editor(df_team2.round({'value': 1}), hide_index=True, key='df_team2')
...
st.write(f"Total value for selected members of {team1}: {selected_team1['value'].sum().round(1)}")
st.write(f"Total value for selected members of {team2}: {selected_team2['value'].sum().round(1)}")